...
LONG _declspec(dllexport) EnableCaption(LONG bCaption)
{
    ...
    if (hTop)
    {
        if (!bCaption)
        {
            SetWindowLong(hTop, GWL_STYLE, GetWindowLong(hTop, GWL_STYLE) & ~WS_CAPTION & ~WS_BORDER);
        }
        else
        {
            SetWindowLong(hTop, GWL_STYLE, GetWindowLong(hTop, GWL_STYLE) | WS_CAPTION | WS_BORDER);
        }
        SetWindowPos(hTop, NULL, 0, 0, 0, 0, SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOMOVE | SWP_FRAMECHANGED);
    }
    return GetLastError();
}
